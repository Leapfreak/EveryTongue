...
## Group 1 — PipelineRunner: the same pipeline written three times  ✅ DONE 2026-07-30
...
## Group 4 — Concurrency runners' statistics math  ✅ DONE 2026-07-30
...
## Group 5 — Benchmark CSV export  ✅ DONE 2026-07-30
...
## Group 6 — TranscribeController's twin catch ladders  ✅ DONE 2026-07-30
...
## Status 2026-07-30
Groups 1, 4, 5, 6 extracted (PipelineRunner: DownloadStep/TrimStep/ChunkStep/
CleanupChunks/CleanupPreview/ValidateWhisperTools; LatencyStats; AppendStageCsv;
OnPipelineCancelled/Error/Unexpected). Groups 2 and 3 remain scheduled after
Sunday. The residual PipelineRunner match is the step-header CALLER skeleton
(Report/Log + one-line helper calls with mode-specific step numbers) — kept
deliberately for readability.

## Round 2 — revealed when the top-25 cap cleared (all smaller, none urgent)
- Translation backend family: CloudTranslationBackend ↔ OpenAiCompatibleBackend
  ↔ Amazon/LibreTranslate share the per-target loop shape (live path — treat
  with Group 2 discipline, after Sunday).
- EndpointRegistration.Settings.vb: internal endpoint-shape repetition.
- live-server vad/: pipeline.py ↔ accumulating_pipeline.py share worker
  scaffolding; segment.py internal pair.
- Template forms: FormDisplayTemplates/FormEngineTemplates/FormTemplateManager
  share a combo-population block.
- ConversationAudioHandler: two internal blocks.
- TranscribeController: the Run/Resume launcher PREAMBLE (distinct from the
  extracted catch ladders).
