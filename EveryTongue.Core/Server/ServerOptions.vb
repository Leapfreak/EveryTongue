Namespace Server
    ...
    Public Class ServerOptions
        ...
        ''' <summary>
        ''' Head-supplied factory for the local TTS playback sink: (deviceNumber, volume) →
        ''' a started-able sink. The Windows head passes an NAudio TtsAudioOutput here;
        ''' headless hosts leave it Nothing (no local playback — phones still play TTS).
        ''' Keeps NAudio out of the server core.
        ''' </summary>
        Public Property TtsSinkFactory As Func(Of Integer, Single, Services.Audio.ITtsAudioSink)

        ''' <summary>
        ''' Head-supplied enumerator for local audio OUTPUT devices (the /tts/devices
        ''' endpoint). Nothing = empty list (headless hosts have no local audio).
        ''' </summary>
