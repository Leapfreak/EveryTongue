...
Namespace Server.Hubs
    ...
    Public Class SubtitleHub
        ...
        Public Async Function HandleAsync(context As HttpContext) As Task
            ...
            Finally
                ' A disconnecting broadcaster releases the room's web-mic slot so the
                ' host can re-claim from a reloaded page (health flips capturing=false
                ' via the frames-flowing window, so readiness banners stay honest).
                If client.IsBroadcasting Then
                    client.IsBroadcasting = False
                    If WebMicRouter.Instance.ClearBroadcaster(client.RoomId, clientId) Then
                        AppLogger.Log(LogEvents.CONF_WEBMIC, $"room={client.RoomId} broadcaster {clientId} disconnected")
                    End If
                End If
            ...
            Try
                While client.WebSocket.State = WebSocketState.Open AndAlso Not ct.IsCancellationRequested
                    ...
                    ElseIf result.MessageType = WebSocketMessageType.Binary AndAlso result.Count > 0 Then
                        ...
                        If client.IsBroadcasting Then
                            ' Continuous web-mic PCM. An 8-byte "ETMC" header frame declares
                            ' the format (version/codec) and is consumed here, not forwarded —
                            ' the live-server ingest expects raw 16kHz mono s16 frames only.
                            If audioData.Length = 8 AndAlso audioData(0) = &H45 AndAlso audioData(1) = &H54 AndAlso
                               audioData(2) = &H4D AndAlso audioData(3) = &H43 Then
                                _logger.LogDebug("Web-mic header from {Endpoint}: version={V} format={F}",
                                    client.RemoteEndpoint, audioData(4), audioData(5))
                            Else
                                ' Sent inline (not fire-and-forget): frames are ~100ms apart and the
                                ' router's localhost send is sub-ms — ordering preserved for free.
                                Await WebMicRouter.Instance.SendFrameAsync(client.RoomId, audioData, ct).ConfigureAwait(False)
                            End If

                        ' Process audio in background (don't block the read loop)
                        ElseIf Not String.IsNullOrEmpty(client.RoomId) AndAlso _audioHandler IsNot Nothing Then
            ...
            Try
                Using doc = JsonDocument.Parse(msg)
                    ...
                    Select Case msgType
                        ...
                        Case "broadcastStart"
                            HandleBroadcastStart(client)
                            Return True

                        Case "broadcastStop"
                            If client.IsBroadcasting Then
                                client.IsBroadcasting = False
                                Services.Rooms.WebMicRouter.Instance.ClearBroadcaster(client.RoomId, client.Id)
                                AppLogger.Log(LogEvents.CONF_WEBMIC, $"room={client.RoomId} broadcaster {client.Id} stopped")
                                SendToClient(client.Id, "{""type"":""broadcastState"",""active"":false}")
                            End If
                            Return True
        ...
        ''' <summary>
        ''' Web-mic broadcastStart: HOST-ONLY (the host token machinery establishes
        ''' HostClientId; anyone else is rejected — an open ingest would let any
        ''' joiner speak into the room's STT/translation spend). One broadcaster per
        ''' room, last-claimer-wins: a second start from another device of the same
        ''' host takes over and the previous broadcaster is told to stop capturing.
        ''' </summary>
        Private Sub HandleBroadcastStart(client As ClientConnection)
            Dim roomId = client.RoomId
            If String.IsNullOrEmpty(roomId) Then
                SendToClient(client.Id, "{""type"":""broadcastState"",""active"":false,""error"":""not-in-room""}")
                Return
            End If
            Dim room = _roomManager.GetRoom(roomId)
            If room Is Nothing OrElse room.HostClientId <> client.Id Then
                AppLogger.Log(LogEvents.CONF_WEBMIC, $"room={roomId} broadcastStart REJECTED for {client.Id} (not host)")
                SendToClient(client.Id, "{""type"":""broadcastState"",""active"":false,""error"":""not-host""}")
                Return
            End If
            If Not WebMicRouter.Instance.IsWebMicRoom(roomId) Then
                AppLogger.Log(LogEvents.CONF_WEBMIC, $"room={roomId} broadcastStart rejected — room's engine not started with audio_source=web")
                SendToClient(client.Id, "{""type"":""broadcastState"",""active"":false,""error"":""not-web-mic-room""}")
                Return
            End If

            Dim previous = WebMicRouter.Instance.SetBroadcaster(roomId, client.Id)
            If Not String.IsNullOrEmpty(previous) Then
                Dim svc = TryCast(_subtitleService, SubtitleService)
                Dim prevClient = svc?.GetClient(previous)
                If prevClient IsNot Nothing Then prevClient.IsBroadcasting = False
                SendToClient(previous, "{""type"":""broadcastState"",""active"":false,""error"":""taken-over""}")
                AppLogger.Log(LogEvents.CONF_WEBMIC, $"room={roomId} broadcaster takeover: {previous} → {client.Id}")
            Else
                AppLogger.Log(LogEvents.CONF_WEBMIC, $"room={roomId} broadcaster started: {client.Id}")
            End If
            client.IsBroadcasting = True
            SendToClient(client.Id, "{""type"":""broadcastState"",""active"":true}")
        End Sub
