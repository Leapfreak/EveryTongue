...
Namespace Server
    ...
    Public Class KestrelHost
        ...
        Private Function BuildApplication(options As ServerOptions,
            ...
            ' Wire TTS audio output for local PA/NDI playback. The sink comes from the
            ' head via ServerOptions.TtsSinkFactory (NAudio lives in the Windows head,
            ' not the core); no factory = no local playback (headless).
            Dim subtitleSvc = TryCast(app.Services.GetService(GetType(ISubtitleService)), SubtitleService)
            If subtitleSvc IsNot Nothing AndAlso options.TtsOutputDevice >= 0 AndAlso options.TtsSinkFactory IsNot Nothing Then
                Dim ttsOutput = options.TtsSinkFactory(options.TtsOutputDevice, options.TtsOutputVolume)
                If ttsOutput IsNot Nothing Then
                    ttsOutput.Start()
        ...
        Private Sub ConfigureServices(services As IServiceCollection,
            ...
            services.AddSingleton(Of ITranslationBackend, DeepSeekBackend)()
