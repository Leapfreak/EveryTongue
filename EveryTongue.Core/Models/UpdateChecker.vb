...
Namespace Models
    ...
    Public Class UpdateChecker
        ...
        Private Const Repo As String = "EveryTongue"

        Private Shared ReadOnly _client As New HttpClient()

        Shared Sub New()
            _client.DefaultRequestHeaders.UserAgent.ParseAdd("EveryTongue-UpdateChecker")
        End Sub
        ...
        Public Shared Async Function CheckForUpdateAsync() As Task(Of UpdateInfo)
            Try
                ...
                Using doc = JsonDocument.Parse(json)
                    ...
                    ' Entry assembly = the HEAD app (EveryTongue.exe / Lite) — this code now
                    ' lives in Core, whose own version would understate the app's.
                    Dim localVersion = If(System.Reflection.Assembly.GetEntryAssembly(),
                    ...
                    If root.TryGetProperty("assets", assetsEl) Then
                        For Each asset In assetsEl.EnumerateArray()
                            ...
                            If name.StartsWith("EveryTongue_Setup", StringComparison.OrdinalIgnoreCase) AndAlso
                               name.EndsWith(".exe", StringComparison.OrdinalIgnoreCase) Then
                                info.InstallerUrl = assetUrl
                            ElseIf name.StartsWith("EveryTongue_App_", StringComparison.OrdinalIgnoreCase) AndAlso
                                   name.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) Then
                                info.AppZipUrl = assetUrl
                            ElseIf name.StartsWith("EveryTongue_Whisper_", StringComparison.OrdinalIgnoreCase) AndAlso
                                   name.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) Then
