...
async def start_capture_endpoint(request: Request):
    ...
    # callback — despite latency="high" + the torch thread cap, Jezer lost the
    # whisper session's OPENING sentence to "input overflow" twice
    # (2026-07-29), and the Speechmatics room logged the same overflow during
    # the load window (survived by luck). BOTH engines capture audio locally
    # in this process, so both get the synchronous load: session start costs
    # ~25s more — invisible when rooms are started before the service — and
    # the first phrase can no longer be the price. Rooms already gate
    # "STT ready" on capture, so nothing user-facing happens earlier anyway.
    ...
    vad_config = VadConfig(
        ...
        eou_auto_tune=body.get("eou_auto_tune", False),
    )
