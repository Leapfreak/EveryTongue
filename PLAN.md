...
- **Profanity now masks the SOURCE-language captions** (was translation-output-only — field log: "You fucked this up." reached the English feed raw because source-language viewers bypass the translation pipeline). `MaskSourceProfanity` in ConferenceController (all 3 commit paths: immediate + garbage broadcasts, and the original-text field in every BroadcastTranslated) + ConversationAudioHandler (global profanity file; members reading the speaker's language + shared-device source entry). **Dictation rooms exempt** (user's own document). Translation INPUT stays raw (engines need real words; their output was already masked). **Known gap: live partials are unmasked** — a word can flash 1-2s before the masked commit replaces it.
