# EveryTongue — TODO (updated 2026-08-02, **v2.12.0 — released** (evening after the 08-02 service): git + tag + GitHub Release incl. installer + app zip + manifest + aws-sdk carry-over; **ghcr :2.12.0–:2.14.0 + :latest PUSHED 2026-08-11; :2.15.0 PUSHED 2026-09-03; :2.15.1 + :latest PUSHED 2026-09-04** (multi-arch amd64+arm64, each built from its git tag; also fixed :latest having been amd64-only since 2.11.0). The service-log-driven batch: scripture-ref detection 2/24→24/24, post-close feedback page (event 5109), the roomClosed-never-delivered fix — see the v2.12.0 block below. Jezer re-sync target is now **v2.15.1** (engine residency + fast room start + the silent-load heartbeat fix; see the v2.15.0 block below). Prior releases: v2.11.2 (pre-service fix batch), v2.11.0 (names stack), v2.10.x, v2.9.0, v2.8.x — v2.8.x history: v2.8.0 = three-page IA restructure; **v2.8.1 = CRITICAL fix**: the IA surgery doubled `<div id="panel">` in index.html, nesting the whole room UI (captions/picker/Bible) inside the hidden settings panel — captions received+rendered but INVISIBLE. Found via field instrumentation in 4 runs (received/rendered counters → visual-state heartbeat → ancestor chain named `container[0x0] inside panel[0x0 none]`). **FIELD-CONFIRMED FIXED** (session 20260716_2025: 43-pass integrity, pickLang→picker_hidden, captions ONSCREEN). **v2.8.2 = cleanup**: diagnostics now opt-in via `?diag=1` (or localStorage etDiag=1) — badge/heartbeats/picker-logs off by default, anomaly reporting (render errors, skipped commits, ws errors) always-on; one-shot surgery scripts deleted. `tools/render-test.js` (real app.js in headless Edge, runs with diag=1) asserts PHYSICAL visibility — container parent must be BODY + nonzero on-screen rect. LESSONS: scripted HTML surgery needs a tag-balance check; "in DOM = rendered" was the harness blind spot; **the startup integrity line (`43 pass, 0 fail`) is the deploy truth-test — a 1-fail wwwroot deploy cost 3 debugging rounds because the warning scrolled past. Follow-up ADDRESSED (verified 2026-08-02): failures now log per-file [FAIL]/[MISSING] on dedicated STARTUP_INTEGRITY_FAIL (can't be rate-limiter-buried); full report via Tools → Verify File Integrity. Startup stays log-only by design — no self-heal/hard-block built.**)

**Engine residency & fast room start — RELEASED as v2.15.0 (2026-09-03: git + tag + GitHub Release incl. installer + app zip + manifest + aws-sdk carry-over) and FIELD-VALIDATED the same day on the laptop that motivated it** (session 20260902_210037, RTX 2070 Super 8GB — the machine whose four silent no-caption rooms on 2026-09-02 started this): **`room ready in 2031ms — stt=1281ms sttSpare=hit translation=0ms`** (was ~20-28s good-case, infinite-silence bad-case), Salamandra 286-931ms/contexted sentence on the 8GB card, room close parked the live-server+SaT as the warm spare. Plan doc ENGINE_CONCURRENCY_PLAN.md deleted after completion (the CONFIG_CHANGES.md convention) — full design, decisions, and deviations are in its git history. One-paragraph summary: `SidecarReadiness` progress-aware waits replaced all seven wall-clock give-up deadlines (only knob: `EngineLoadIdleTimeoutSeconds`, NO hard cap — a visibly-loading engine keeps its slot, a dead process fails immediately); `EngineResidencyArbiter` enforces `MaxConcurrent{Stt,Translation,Tts}Engines` (default 1, Options → Advanced, en/es/ca) with lease-0 warm spares (NLLB stays resident after room close, llama-server evictable instead of squatting 4.2GB), translation re-routes / STT refuses at the limit, global engine re-warms when idle; live-server `/warm` + park-at-close + app-start sequential warm queue (one heavy load at a time; Lite parity) give ≤3s room starts; CONF_ROOM_READY 5018 timing line + status-bar engine chip + per-port live-server logs (`--log-name` on both python servers — also fixed pooled translate-server tails that never matched their file); `audit-static-timeouts.js` + `tools/ReadinessTest` (17/17). **ghcr :2.15.0 + :latest pushed 2026-09-03 (multi-arch, digest-shared). Remaining: full-Sunday endurance run; field spare-reuse on a second room.** **v2.15.1 (2026-09-04) — silent-load heartbeat bug fix.** Jezer session 20260903_205959: the cold SaT load emits NO log lines for its whole ~21s, `SidecarReadiness`'s only life signal is log-tail advance (15s idle window) → NO PROGRESS, room sent ready FAIL-OPEN 11s before capture actually started — the exact false-ready class v2.15.0 exists to prevent, missed in field validation because that room hit the warm spare (sttSpare only pre-warms ONLINE engines by design; local whisper rooms always cold-load SaT). Fix: `_load_heartbeat(label)` context manager (thread-based so it works even with the event loop blocked) logs "<label>: still loading (Ns so far)" every 5s around every silent model load — live-server SaT (/start + /warm) + faster-whisper WhisperModel, translate-server ctranslate2 /load, mms-tts from_pretrained (also covers its first-use DOWNLOAD). whisper-server/llama-server need nothing (their own output is already tailed).

**Translation context system v1 — RELEASED as v2.13.0 (2026-08-04: merged to main, tagged, GitHub Release incl. installer + app zip + manifest + aws-sdk carry-over). THE BIGGEST TRANSLATION-QUALITY ADVANCE SINCE THE PROGRAM BEGAN — the first change that beat engine-parity, because it changed the INPUT REGIME instead of the engine.** Live-validated on Jezer 2026-08-03 (sessions 20260803_224118 DeepL+context vs 20260803_224440 NLLB, the gomets passage that defeated every engine in July): **DeepL+context 17/21 clean / 4 soft / 0 critical @ 218ms mean — NLLB 6/22 clean / 9 soft / 7 CRITICAL ("wheel system"/"cup"/"yellow gum"/"red goblin"/"elbows") @ 653ms mean.** Headline proofs: bare 3-word fragment "Sistema de gomets." → "A sticker system." purely from ctx=4s; "Em posaré dos"→"I'll put ON two" (referent from context); DeepL beat its own July bare baseline ("golden star"→"golden sticker"). Residual DeepL softs ("yellow star"/"red dot") = lexical-consistency drift = exactly the terminology channel's job (file ships EMPTY by owner decision — clean tests; add entries when the test phase ends). Mechanism recap: the bake-off's "remaining errors are context-starvation" conclusion, made mechanical. Per-room rolling window of recent committed source sentences (`RoomTranslationContext`, snapshot-and-append under one lock at the `TranslateTargetsAsync` chokepoint — current sentence never in its own window) + terminology channel (`translate-server/terminology.json`, mtime-cached `TerminologyStore`, entries matched per sentence; seed file EMPTY by user decision — no gomet entry, clean tests). `ITranslationService`/`ITranslationBackend` gained trailing `Optional context`; orchestrator strips it for backends whose registry entry lacks `SupportsContext` (only `deepl` = True today: prior sentences → free `context` form param; matched terms → `custom_instructions`, target families de/en/es/fr/it/ja/ko/zh only, HTTP-400-retry-without-instructions fail-safe since the repeated-form-key wire format is docs-verified only). Pivot second hops get no context (source-language window is wrong content for English input). Shadows receive the SAME snapshot (stripped per engine) — `ctx=Ns/Mch` marker on 4005/4012 metadata lines = "window existed"; registry says who used it. New event TRANS_CONTEXT 4015 (terminology parse fail / instructions rejected). Config: `TranslationContextEnabled`(on)/`Sentences`(4)/`MaxChars`(700)/`MaxAgeMinutes`(5)/`TranslationTerminologyEnabled`(on)/`TerminologyPath`. Capability matrix gained the translation-backends table (registry-marker-probed). **Deviations from plan:** ctx marker NOT on the nllb-direct 4005 line (that path can't receive context — marker would lie); inline-covered commits append to the window but their :1250 shadows fire without ctx (edge path — inline translation engine is retired). **Deferred:** LLM sidecar backend (SalamandraTA — Jezer GGUF test pending; **its Python cache key MUST gain a context hash or contexted results alias**), FormOptions UI + locale keys, per-engine window overrides, Google/Azure context adapters (no API support). **Remaining validation:** a full Sunday service with DeepL primary + `nllb-3.3b-int8,google-translate` shadows — the gomets test proved the vocab/disambiguation classes; the Papa→Pope/pronoun/verse-quoting class needs real sermon length. Watch first-service latency for any custom_instructions engagement once terminology entries exist. **Open release chores: Jezer re-sync target is now v2.13.0 (superseded: v2.14.0); Lite ghcr :2.12.0/:2.13.0 pushed 2026-08-11.**

**Salamandra offline translation engine — RELEASED as v2.14.0 (2026-08-04: merged, tagged, GitHub Release incl. installer + app zip + manifest + aws-sdk). FIELD-VALIDATED on Jezer same day: two consecutive error-free conference runs (sessions 08:04, 08:59 — Errors=0), 33/33 layers offloaded to the RTX 4080 SUPER (llama-server verbatim), 113-668ms/contexted sentence, metaphor intact end-to-end. Field fixes folded in during validation: llama-server timestamped-log parsing; trailing-space prefill removed (root cause of echo repeats + lost captions + the don-apostrophe-Cyrillic artifact) + degenerate-output bare-retry guard; explicit repeat_penalty=1.0 (server default penalized prompt-present tokens); FACTS-ONLY device reporting (--list-devices + -lv 4 offload lines verbatim; no inferred GPU claims — owner rule); sampler dumps to Debug. Remaining quality tail (tuning, not defects): sticker/card/dot lexical drift, short-fragment context bleed (et contesten → negation carryover).** the DeepL succession plan made real. New engine key `salamandra` ("SalamandraTA 7B (offline, ~6 GB VRAM)") = llama-server.exe (llama.cpp **b10242 Vulkan** — the standing Vulkan-not-CUDA decision, user-benchmarked) serving BSC's salamandraTA_7B_inst_q4.gguf on port 5097, wrapped by `Pipeline\LlamaServerHost` (idempotent shared-task start; both pipes drained line-by-line; Vulkan device + offload lines parsed and logged on TRANS_LLAMA 4016, CPU-fallback loudly on TRANS_LLAMA_PROBLEM 4017 — the field-observed silent-CPU failure can't hide; dual warm-up short+context-length pays the one-time large-prompt shader compile at load; restart-once then give-up with orchestrator fallback to Local/NLLB) and `Services\Translation\SalamandraTranslationBackend` (**prefix-forced continuation**: prior source sentences in the user turn + their broadcast translations pre-filling the assistant turn from the v2.13.0 context window — the model completes only the current sentence; validated bare prompt byte-identical to the test kit; stop `<|im_end|>`, temp 0, `cache_prompt` for rolling-prefix KV reuse; **no response caching → no context-hash aliasing possible**, the standing cache-key warning is discharged by design). Registry: `SupportsContext=True`, **`SupportsTerminology=False` (tested: the model ignores prompt glossaries — glossary/profanity fixes run via the orchestrator post-processors instead, `AppliesFiltersInternally=False`)**, new `Entry.LocalServerKind="llama"` discriminator + `IsOfflineEngine()` helper (readiness probe + lobby badge switched; **ConferenceController buffering split deliberately stays ModelType-keyed** — Salamandra takes the immediate per-commit path, buffering is an NLLB artifact). Wiring: ServerController constructs/registers once at server start (both heads), ensure-on: server start when configured, room acquire, Options save (live switch, no restart), engine-aware benchmark start (was NLLB-only and would wait forever); `TryActivateConfiguredCloudBackend`→`TryActivateConfiguredBackend` (gate = ModelType, fixing Translate-workspace/batch silently running NLLB when salamandra selected); orphan sweep + FormClosing stop. Languages: `salamandra` presence column on 40 rows of language-codes.json (Aragonese absent from the 167-table — nothing to mark); unsupported source/target → loud TRANS_ERROR omit (per-target fallback rerouting = deferred). Download Manager: "Salamandra Translation Model" (5.07 GB GGUF via embedded-python `hf_hub_download` — resumable, 2h timeout; direct-stream fallback) + "llama.cpp Server (Vulkan)" (pinned b10242 zip → llama-bin\, shared exe-path helper with ServerController); Piper/llama zip-hoist extracted to shared `HoistNestedDirectory` (audit-clones clean). **Deferred:** FormOptions knobs + locale keys (config file-only, audit-config allowlisted), per-target fallback rerouting, Lite/Linux llama binaries, extract-shared process-host base (trigger: a third host species). **Validation next (Jezer):** DM install both components (incl. kill-and-resume the 5 GB download), select engine in Options, 4016 device/warm-up lines, dictation smoke, conference room en+es (room holds "preparing" until warm; 4005 `backend=Salamandra ctx=` from commit 2; prefix continuation = only-current-sentence output), glossary via post-processor, crash drill (kill llama-server mid-room → 4017 + fallback + recovery), unsupported-target omit, FLORES Pair A/B vs NLLB onto pair-scores.csv, exit/orphan sweep.

**v2.8.3 — RELEASED 2026-07-17 (git + ghcr + GitHub Release) — evening-session batch, all field-checks pending:**
- **Web-mic accidental-exit guard** — `beforeunload` warning while this device is the room mic (`bcWant||bcActive`); **End Room now confirms** ("End this room for everyone?"); deliberate exits (dictation ✕, confirmed End Room) stop the broadcast first so they never double-prompt. Keys `web.bcLeaveWarn`/`web.endRoomConfirm` in en/es/ca. Context: b65715e (committed) separately fixed the 40s forwarder drop — field-check = event 5017 only on REAL live-server restarts.
- **Speechmatics language/target change = in-place WS reconnect** (speechmatics.py) — rides the EOU auto-tune's reconnect loop instead of `stop()`+`start()`: capture stream never closes (kills the "input overflow" warning + the audio gap; words spoken during the swap are buffered, not lost). Also fixed en route: `_reset_pending` was only honored when auto-tune was ON; biblical vocab now reloads for the NEW language (was stale after a switch); `_normalize_lang` maps ISO synonyms (zh→cmn) in session + one-shot paths. **Same stop/start pattern still lives in deepgram/gladia/azure_speech** (no reconnect loop to reuse) — restructure on demand. Field-confirmed same evening: log shows `reconnecting session in-place`, commits flow across the ca→en switch. NOTE: a real "input overflow" can still appear once at genuine CAPTURE START (device open priming) — that one is honest.
- **Dictation follows the standard room protocol** — lobby's speaking-language dropdown REMOVED (`sourceLang` optional in POST /api/rooms, kept for API compat); the room-entry picker is the ONE language question: it sets the client caption language as normal AND retunes the room's STT via host-gated `POST /api/control/pipeline` (`applyDictationSpeakLang`, FLORES→ISO via LANGS; race-covered when picking before room info). Fixed en route: **the dictation editor was PAINTING OVER the open picker** (both z-index 50, editor appended later → "auto-enters before language pick") — editor now z-index 40. **Dictation is now a minimal solo editor**: `#status` bar hidden (no home/settings/Bible/QR), `showHostControls` skipped, Done button → ✕ (close room → lobby). Known gap: connection-drop text lived in the hidden bar — a blip mid-dictation is silent (auto-resume still works).
- **Lobby reordered**: Host Conference (main weekly flow) → Create Conversation → Dictation (bottom); each section carries its own trailing divider inside its wrapper so hidden sections (no templates / Lite without offline engine) leave no stray lines.
- **Static language lists ELIMINATED** (standing rule now enforced everywhere): `language-codes.json` gained **googleStt/azureStt** BCP-47 vendor columns; python engines read them via `common.vendor_locale()` (google.py `_WHISPER_TO_BCP47` + BOTH `_LATEST_*` model lists deleted — model choice now self-heals: try latest_*, drop to default on vendor rejection incl. a new streaming-error rung; azure `_LOCALE_MAP` deleted, xx→xx-XX heuristic kept). VB: new **`SttBackendRegistry.EffectiveLanguages(key)`** = the /api/stt-languages logic extracted — endpoint, FormMain `_sttLanguages` (was a 100-entry array; now auto + whisper column) and FormTemplateManager (was 23 hardcoded) all call it. app.js host-panel 13-option fallback deleted (`populateSttLangs` gained a `desired` param so the room's language still pre-selects). Latent bug fixed: old google map keyed Norwegian as `nb` (whisper says `no`) — Norwegian never mapped; `no`/`nn` rows now carry nb-NO.
- **Profanity now masks the SOURCE-language captions** (was translation-output-only — field log: "You fucked this up." reached the English feed raw because source-language viewers bypass the translation pipeline). `MaskSourceProfanity` in ConferenceController (all 3 commit paths: immediate + garbage broadcasts, and the original-text field in every BroadcastTranslated) + ConversationAudioHandler (global profanity file; members reading the speaker's language + shared-device source entry). **Dictation rooms exempt** (user's own document). Translation INPUT stays raw (engines need real words; their output was already masked). **CORRECTED 2026-07-17: the "unmasked partials" gap does NOT apply to rooms** — `BroadcastUpdate` skips all room-assigned clients (SubtitleService), so room viewers only ever see commits, which are masked. ~~The one remaining unmasked surface is the LEGACY bare-URL live view~~ — CLOSED 2026-08-02 by code inspection: `BroadcastUpdate` (the unmasked no-room update path) has zero callers left, and every `BroadcastCommit` site is room-scoped in ConferenceController with masked text. The legacy no-room feed is dead code; nothing unmasked reaches any client.
...
1. **Smoke-test the new cloud engines with real API keys** (test machine) — Deepgram/Gladia/Azure Speech STT, DeepSeek/OpenAI/LibreTranslate/Amazon translation, Azure/Google/OpenAI TTS were built from vendor docs only; verify auth, response shapes, and fallback behavior per engine, fix against real responses. (The Speechmatics + NLLB-3.3B + Piper stack is proven.) **ALSO in the same session (agreed 2026-07-17): restructure deepgram/gladia/azure_speech language-change to the Speechmatics in-place-reconnect pattern** — they still `stop()`+`start()` the capture stream on a language change (mic teardown + audio gap); deliberately NOT refactored blind since the engines can't be run without keys. All three vendors have free tiers if keys are wanted sooner.
...
## Immediate TODO
- [ ] **Clone extractions, post-Sunday batch (2026-07-30, user-approved; CLONE-REPORT.md deleted after reading — this line is the surviving tracker).** Done already: PipelineRunner shared steps, LatencyStats, AppendStageCsv, TranscribeController outcome handlers. G2+G3 DONE 2026-07-31 (user moved them up: RunnerBackedSttBackend base class E2E-verified via fake-mic whisper room — Catalan STT + [EOU-TUNE] first live retune + [WHISPER-HOLD] + DeepL captions rendered; CAH now injects IOptions(Of ServerOptions), and the FormMain API-key push writes the SHARED options instance — the old per-copy push left ServerOptions stale for other readers). Remaining round-2 smaller ones (~24 blocks): translation-backend per-target loop family, EndpointRegistration.Settings internals, vad pipeline↔accumulating_pipeline scaffolding, template-form combo block, sidecar log-writer ×4 (needs a packaging decision). Definition of done per group = `node tools/audit-clones.js` stops printing it.
...
**v2.11.0 released (2026-07-28): git tag + GitHub Release (installer + app zip + manifest + aws-sdk carry-over) + ghcr :2.11.0/:latest. THE names-stack release: two-layer STT vocab (service + book-scoped), scripture detection via derived alias index, Tools → Service Names with multi-format notes import (pdf/docx/odt/pages/rtf/txt/json/xml), log events 6003/6004/3010, BCI integrity false-alarm fix.**

**v2.11.1 released (2026-07-30): git tag + GitHub Release (installer + app zip + manifest + aws-sdk carry-over) + ghcr :2.11.1/:latest. THE Sunday field-hardening release (everything from the 07-29/30 rehearsal logs): whisper EOU auto-tune (shared PaceTuner) + clause-hold/SaT ON by default + SaT pre-capture load BOTH engines; web-mic on ALL cloud engines; service-names → whisper initial_prompt; Speechmatics concurrent-quota hardening (no reconnect for pre-connect vocab push, 2s slot-release grace, 3s non-counting quota retry); room restart honors current language; scripture detection rounds 1-3 (announcement phrasing, abbreviation exclusion, chapter-range validation, locale chapter words); ca hallucination ghosts; pypdf via DM (requirements-derived); 14-auditor suite + 6 clone-group extractions + 225 silent handlers logged. THIS is the build for the Jezer re-sync — more testing 07-31, service 08-02.**

**v2.11.2 released (2026-08-01): git tag + GitHub Release (installer + app zip + manifest + aws-sdk carry-over) + ghcr :2.11.2/:latest. Pre-service fix batch (from the 07-31 testing evening), now the Jezer re-sync build: (1) inline-translation × Hold&merge/SaT conflict — silently produced ZERO subtitles since v2.11.1 turned hold/SaT on by default; room start now detects it, falls back to the default translation engine, warns via new event 4014; (2) live GGML default flipped turbo → full large-v3 (quality) with once-only config migration (only if the large-v3 file exists; deliberate turbo re-pick never re-flipped) + DM download URL + GGML path in server tool-status; (3) room-needs-local-translation with missing deps was a silent Return → now logs TRANS_ERROR + prompts Download Manager.**

**v2.12.0 released (2026-08-02 evening): git tag + GitHub Release (installer + app zip + manifest + aws-sdk carry-over). ghcr :2.12.0 PENDING. THE service-log-driven release — everything came out of the 08-02 morning log review + same-evening field tests (branch `spoken-scripture-refs`, merged):**
- **Scripture-ref detection overhaul: 2/24 → 24/24 on the replayed 08-02 sermon corpus, 0 false fires.** Four gaps closed, all language data in locales (zero strings in code): (1) singular spoken book names — Bibles title the book plural (Salmos/Salms/PSALMS), preachers cite singular; new key `Bible_SpokenBookNames` (operator data, overrides frequency-derived ambiguity — Wycliffe's lowercase "psalm" prose had demoted English "Psalm 23"); (2) filler forms "Mateo en el capítulo 27" (≤2-connector loosening + chapword-as-book re-anchor + all-suffix leading-drop — the old 2-token cap hid announcement-prefixed refs at BASELINE); (3) spelled-out numbers via `Bible_NumberWords` = FULL 1–200 enumeration per language (es 200 / ca 411 / en 643 entries, generated by `tools/gen-number-words.js`; longest-phrase match, NO arithmetic in code; language-hint scoped — es "once" ≠ English "once"); (4) per-room reading context (new event **6005**): bare "versículo 15" resolves to the last-heard book/chapter, a numberless book word ("del salmo") rescues recency back after a cross-reference — fixes the real 11:26 trap where v15/v18 would have silently shown Matthew 27's verses. Hardening: accent folding across the whole spoken layer (`BookAliasIndex.Fold` — no accent-doubles in data), capitalized chapter/verse words, lowercase book names gated on frequency-proven non-ambiguity, punctuation-tolerant phrases. RefPreview harness: real 387-line sermon corpus + 35 must-hit/must-miss probes + the context-sequence test, exits nonzero on failure. Field-confirmed same evening (6003 fired on "Salmo 22"). NOT covered (open): ordinal-word verses ("el primer versículo") — could ride `Bible_Ordinals` later.
- **Post-close audience feedback page — first slice of plan #11A-a:** conference-room guests get a problem-framed page when the room closes ("Did the subtitles give you any trouble today?", 1–5 stars, comment placeholder naming concrete failure kinds, "No problems — skip", 90s idle timeout; host exempt; **Save Transcript moved here for conference guests**, menu copy hidden for them only). POST `/api/rooms/{id}/feedback` validates against the closed Room object (lingers ~1h before purge), one submission per client → **one session-log line, event 5109**: room, name, rating, viewer language (FLORES form — normalize to plain code if it grates), `device='Android 10 / Chrome'` from the request UA via the existing `ParseUserAgent`. Locale keys ×3; **es/ca wording is Claude's — native-speaker read-over before Sunday**. Field-confirmed: real 5/5 submission from the Android test phone.
- **THE roomClosed-never-delivered fix:** `CloseRoom` marks the room inactive BEFORE `hub.BroadcastToRoom`, whose first line was an active-only `GetRoom` → silent early return — **the roomClosed message had never reached a single client since rooms were introduced** (explains the 08-02 12:09 service trace and every "clients still connected after close" report; zero log evidence because the miss path logged nothing). Fix: closed-inclusive lookup; the broadcast returns + logs its reach ("roomClosed broadcast reached N client(s)", 5101); clients answer with a `roomClosed handled` clientLog receipt — field logs now prove delivery end-to-end. Defense-in-depth for sleeping phone sockets: GET /api/rooms/{id} on a recently-closed room returns **410 Gone** {closed,name,type} instead of 404, and the client re-checks on every WS reconnect — a missed broadcast self-heals into the same close flow (feedback page included). Also en route: feedback overlay z-index raised above the language picker (50 vs 30 — was invisible behind an open picker), stale-cache fallback to the classic ended-notice + clientLog report. Proven with a scripted live WebSocket guest; 16:13 field test shows reach=2 + both receipts + the 5109 submission.
- **Grace auto-tune log lie fixed:** tuner now skips template-pinned rooms (it logged 1400→3000ms retunes that `CurrentThresholds` deliberately never applies — all 4 rooms on 08-02 logged no-op tunes) and the room scoreboard (5015) reports the EFFECTIVE grace, not the never-applied adaptive value.
- **OPEN for this release:** ~~push ghcr :2.12.0 (+ :latest)~~ pushed 2026-08-11; Jezer re-sync to v2.12.0; Sunday live-validation — watch 6003 vs 6005, 5109 submissions, and "broadcast reached N" + client receipts on every room close (alias cache rescans once ~0.5s/Bible on first start, reported via 6004); native-speaker pass on the es/ca feedback strings.
...
**A. Sunday service on Jezer (desktop v2.11.1)** — prereqs: re-sync bin\Publish (or in-app updater — manifest now at 2.11.1; expect the 50-file integrity pass), biblical vocab already regenerated, **PRUNE the Service Names list** (delete DIAPO/Potser/Cartes/Evangeli/Senyor/Esperit/Fill/Salmo; recase ALL-CAPS entries e.g. ANTONIO CRUZ→Antonio Cruz; Llum only if a person) and **add `Joni Eareckson = Joni Erikson, Jonny Eriksson`**, run tools/fix_spavbl.py if Spanish Bible installed:
...
- [ ] 8. Speaker-language change mid-service: log shows in-place reconnect, no input-overflow, no lost words (speak through the swap), session vocab count matches the NEW language. **v2.11.1 addition:** if "Concurrent Quota Exceeded" appears, expect "retrying in 3s (audio keeps buffering)" and a recovered session — never a dead room (field 2026-07-31 20:47: ca→es change killed the room; if the retry warning repeats >15s, something ELSE holds a session on the account — close it, don't wait)
...
**A2. New in v2.11.1 (from the 07-29/30 rehearsal findings — replay/verify on Jezer 07-31):**
- [ ] 42. Scripture announcement forms detect: "Mateu capítol 4" and "Mateu, capítol 4." → verse/chip; "Amb disset anys" → NO chip (trailing-drop restricted to chapter words); "Am 10" → NO chip (abbreviations excluded live + chapter-range validation). The 📖 chip on translated feeds is DELIBERATE (inline underline impossible there).
- [ ] 43. SM vocab push at room start does NOT reconnect: log shows "session still connecting — the pending session picks this up", then ONE "Session started" with vocab=N (N = pruned list size). Then the Joni passage: "Joni Eareckson" spelled right (the 07-30 "Jonny" session was likely vocab-less from the quota race).
- [ ] 44. SaT pre-capture load, BOTH engines: no "[input overflow]" at room start, opening sentence present in captions on the FIRST pass (was lost 3× in the field before the fix).
- [ ] 45. Whisper room realtime on Jezer: auto-tune retunes visible ([EOU-TUNE], in-place, no restart) + SaT hold gluing guillotine cuts ([WHISPER-HOLD]) at real GPU cadence (dev GPU too slow for cadence realism — this is the remaining gate on the whisper-as-fallback story).
- [ ] 46. pypdf on Jezer via Download Manager (Python Packages check now derives from requirements): run-sheet PDF import extracts real names, not garbage; missing package shows as installable, never "Up to date".
...
**Field / ops:**
- [x] ~~Jezer: clean install of v2.11.0~~ — DONE (rehearsals 07-29/30 ran on it; the field findings drove v2.11.1). **Next: re-sync Jezer to v2.11.2** (in-app updater now carries everything — manifest at 2.11.2 incl. the 08-01 fix batch), then section A + A2 above. (`tools/fix_spavbl.py` if the Spanish Bible is installed.)

**Builds (priority order):**
- [x] **★ Clause-hold + SaT for whisper rooms — BUILT + mechanics live-verified 2026-07-29** (same day as the measurement). Config AppConfig.WhisperUseSatHold (**default ON** per user decision 2026-07-29, 300-char runaway guard mirrors SM MaxChars; per-template toggle UseSatHold on whisper-cpp/faster-whisper descriptors, EngineCfg_WhisperSatHold en/ca/es). DESIGN NOTE: the treatment lives IN live-server (vad/pipeline.py), NOT the .NET clause coordinator — the coordinator merges by commit-ARRIVAL gaps (fits Speechmatics 1-2s fragments, can never merge whisper chunks arriving ~15s apart); the pipeline knows audio truth via commit types. state_machine gained SOFT-MAX (8s duration cut, no pause) so SOFT-COMMIT now unambiguously means a real 400ms pause; hold rule: glue on FORCE-COMMIT/SOFT-MAX, flush via sat_segmenter on COMMIT/SOFT-COMMIT (600-char runaway guard, STOP-FLUSH at close). /start flag sat_hold (runner SatHold prop, warms SaT). ISegmentingSttBackend generalizes the /segment reach. LIVE-VERIFIED on Lite (sermon replay via web-mic): holds glue guillotined chunks (35->225 chars), pause flushes emit SaT sentences -> DeepL -> captions rendered. REMAINING: realtime validation on Jezer (dev GPU too slow for cadence realism) + benchmark A/B rerun through the live path. (tools/whisper_sat_ab.py on 6min of the sermon, real FrameVAD + per-chunk whisper-cli vs batch reference): current whisper pipeline 1.35x over-fragmented (66 units @13.0 words vs ref 49 @16.9) — mainly the 15s max-segment guillotine slicing mid-sentence during continuous speech (chunks averaged 14.8s; pre-roll overlap also duplicates words at every cut). With merge-to-pause + SaT: 0.96 ratio (47 units @18.4) = batch parity, same as Speechmatics+SaT field 0.92. Build: route whisper-room commits through the clause coordinator (currently Speechmatics-gated) behind a toggle. Combined with WER tie (5.1 vs 6.0) + web-mic (done) this makes whisper a credible free/offline Speechmatics alternative; remaining after: names via initial_prompt A/B.
- [x] **Web-mic for whisper rooms — BUILT + verified end-to-end 2026-07-29** (same session as the field finding). TWO gaps: (.NET) WhisperCppBackend/FasterWhisperBackend never forwarded config.AudioSource to LiveStreamRunner (CloudStreamingSttBackend did) → /start lacked audio_source; (python) VadPipeline always opened a sounddevice InputStream. Now: VadConfig.audio_source, VadPipeline web mode (no device; feed() slices browser 16k s16 frames into 512-sample Silero chunks with a remainder carry into the same preroll+VAD queue), web_feed_recent() for honest /health capturing, accumulating-pipeline falls back to VadPipeline for web. VERIFIED via tools/verify-webmic.js: sermon WAV through headless-browser mic -> whisper-cpp-vulkan (Catalan transcripts correct) -> DeepL -> Spanish captions rendered; 712 frames forwarded, no 403. En-route field quirk noted: /start jsonBody does not escape backslashes in absolute Windows model paths (relative/forward-slash paths fine).
- [x] **★ Names priming — the proven quality lever. CORE SHIPPED 2026-07-27** (two-layer service+book vocab, Tools → Service Names dialog, multi-format notes import pdf/docx/odt/pages/rtf/txt, sounds_like syntax, push-on-start + live re-push; speaker profiles deliberately NOT a source — moved away from them in favour of EOU auto-tune). **Remaining pieces:**
  - [ ] **Weekly batch-whisper harvest** as a third name source (capitalized tokens from the QA SRT of the recorded service).
  - [ ] **Translation-side name protection** (personal-article+capital passthrough — previewable against the 2026-07-26 351-commit corpus before building).
  - [ ] **Service Names UI for Lite/web** — currently desktop-only; Lite users must edit `ServiceNames` in appconfig.json by hand. A host-controls or admin-page editor would close the gap.
  - Known behaviours to keep in mind: notes import needs a human prune pass (slide markers like "DIAPO" survive the heuristics by design — the user reviews the list); pushing service names to a RUNNING session costs one in-place WS reconnect (~same as an EOU retune, no audio loss).
- [x] **Scripture popup — WEB HALF LIVE + field-verified 2026-07-28** (user tested Matthew 4 in Spanish AND English feeds; found+fixed en route: source-offset underline on translated feeds → per-text re-detection + 📖 chip fallback, '4:0' → whole chapter, 1.5s fixed retry → fast poll). REMAINING: desktop live-view links (BOTH-viewers rule — the RichTextBox live text has no ref links yet).
- [ ] **Benchmark polish batch**: Speechmatics BATCH API for big sweeps (RT session-churn triggered account throttling: 20 consecutive timeouts 06:15-06:21 on 2026-07-26); WER number-normalization data file (digits vs spelled-out biases engine comparisons ~1-2 pts); exclude retry cooldowns from avg-latency stat. ~~sidecar teardown + timeout detail~~ DONE 2026-07-29: Stop() now waits for the graceful /shutdown before killing (the kill always won the race — every planned stop logged code -1); Exited handler splits deliberate stops (stopped cleanly, Info) from crashes (Warning); FLEURS logs every failed clip with engine/config/elapsed/error. Live-verified: Requesting graceful shutdown -> Live server stopped cleanly.
...
- **Cloud-STT engine smoke tests (Deepgram/Gladia/Azure/Google STT)** — the plugin code shipped untested against real vendors; needs API keys (free tiers exist for Deepgram/Gladia). When keys arrive, also: (a) restructure their language-change to the in-place reconnect (validated live for Speechmatics); (b) ~~add the web-mic feed()/audio_source pattern~~ DONE 2026-07-30 (e4c90b8, ported to ALL cloud engines) but UNVALIDATED against real vendors — first key validates via the fake-mic harness (tools/verify-webmic.js pattern).
...
- [x] **additional_vocab PROVEN (2026-07-26 evening, real service audio):** vocab off/on A/B on the Joni segment (tools/sm_vocab_ab.py): OFF jonny×8/jenny×1 → ON **Joni 9/9**. Notes-PDF→noun-extraction (tools/pdf_nouns.py)→vocab loop validated end-to-end. Sermon forensics (tools/sermon_forensics.py): live-vs-whisper 11.6%% divergence ≈ both engines at benchmark form (live hearing at ceiling); fragmentation SOLVED at translation layer (0.92 live units per whisper segment — clause-hold working in field); errors concentrate in NAMES. Context-concatenation for NLLB DISPROVEN (89%% trim-fallback, canary unfixed; tools/context_ab.py).
- [x] **✅ BUILT (2026-07-27) — Service Names (per-service people vocab, Phase C).** Two-layer source-keyed vocab in the Speechmatics engine: **service layer** (session-lifetime people names — survives book AND language changes) + **book layer** (biblical names swapped on scripture-book detection); merged+deduped, service wins collisions; new `service_vocab` /config key forwarded by live-server. Producer: `AppConfig.ServiceNames` (+sounds_like) edited via **Tools → Service Names** dialog (desktop; localized en/ca/es) with **Import from notes…** — `live-server/vocab/notes_names.py` extracts name candidates from **pdf/docx/odt/pages/rtf/txt** (pypdf for PDF, stdlib zip+XML for the rest; heuristics document-derived, zero static per-language lists — capitalized runs, self-derived common-word test, article-strip; validated on Marina's real PDF: Joni Eareckson, Filipencs, Kenosi, Hebreus found). ConferenceController pushes on backend start (after health) + live re-push on dialog OK (`PushServiceVocabToAll`); log event **[3010] STT_SERVICE_VOCAB**. Speaker profiles NOT used as a source (moved away from them in favour of EOU auto-tune). Lite: set `ServiceNames` in appconfig.json (same Core path). REMAINING: weekly batch-whisper harvest as a third name source; validate live next Sunday (expected log chain: 3010 push → engine "vocab service layer: N names").

**v2.10.1 released (2026-07-27): git + ghcr :2.10.1/:latest + GitHub Release (installer + app zip + manifest — updater current). Patch: benchmark hardening from live shakedown, /transcribe decode+error fixes, web host-controls fix, admin Status card. THIS is the build for the Jezer clean install.**
...
- [x] **✅ BUILT + PUBLISHED (2026-07-26, was P1) — English-pivot for weak target languages.** (Origin: two Swedish guests abandoned translation ~9 min in — Google `cat→swe` direct was weak while `cat→eng`/`eng→swe` were strong.) **Design: one routing policy in the orchestrator — no caller anywhere changes; all heads (desktop/Lite/web) get it automatically.**
  - **⚠️ ORIGIN DIAGNOSIS DISPROVEN (2026-07-26, FLORES A/B + log re-read).** Google cat→swe direct = its pivot, IDENTICAL output (chrF 64.7, best engine measured; NLLB 60.1/60.7 tie, DeepL 61.9/61.7 tie) — routing was never the GLS problem. Log re-read (20260627_090239): Swedes listened 10:02:35–10:09:20 to CLEAN cat source translated fine; the real killers were (1) EOU-0.7s fragmentation ("Per què ? Per." / "El catering." per-fragment to swe + EdgeTTS ~1.4s each with rate-suppression ×11/10s), (2) STT vocab garbling ("Gathering"→"catering", "GLS"→"global de Futsal" — the built-but-unproven additional_vocab is the fix; these are the A/B test terms), (3) manual es/ca language-flapping with full session restarts (5 flips incl. es→ca→es in 12s at 10:08 when a speaker switched to Spanish — the in-place-reconnect item + ca/es mixing problem), (4) es-locked STT hispanicizing Catalan ("l'Església"→"las lesiones") during mismatched stretches. Fragmentation is FIXED since (EOU defaults+autotune+clause hold). Pivot feature stays as built (Auto now evidence-tuned: Google/DeepL/LLMs direct, cousins direct) — its remaining value is unmeasured engines/pairs + the reuse-better-English path. NEXT LEVER from the scoreboard: per-language engine override (swe→Google, +4 chrF over NLLB) — needs config+UI plumbing.
  - **Mechanism** (`TranslationOrchestrator.TranslateInternal`): per backend-group, targets are partitioned direct-vs-pivot by `PivotPolicy`; source→English translated once (REUSED for free when English is already a requested target — the common rooms case), then English→all pivoted targets in one batched call. Both hops run inside the same queue slot (re-entering `TranslateAsync` would deadlock the priority queue). Safety net: empty English hop ⇒ direct fallback, never worse than pre-pivot. Applies to the room `backendOverride` path too.
  - **Decision order (Auto)**: measured/seed entry in `wwwroot/data/translation-direct-pairs.json` (16 seed "cousin" pairs, symmetric, optional per-engine `engines` scoping; measured entries carry chrF scores+engine+date) → engine's `EnglishCentric` registry bias (azure/amazon/libre=True; google/deepl/deepseek/openai/**nllb**=False — NLLB flipped 2026-07-28: many-to-many by design + measured cat→swe tie, and the conservative True had nearly the whole picker badged "via English"). Pairs including the pivot language never pivot.
  - **Config**: `AppConfig.TranslationPivotMode` enum Off/Auto/Always (default **Auto**) + `TranslationPivotLanguage` (default `eng_Latn`, no UI) → `ServerOptions` via the shared `ServerController.StartServer` (Lite needs zero extra wiring). Desktop UI: one combo in FormOptions Translation panel (localized en/ca/es, `Opt_TransPivot*`).
  - **Visibility**: `GET /api/translation/routing?source=&targets=` explains per-target route/reason from the SAME `PivotPolicy` instance the orchestrator uses (cannot drift); `TRANS_PIVOT` (4013) logs the policy summary at startup + every pivoted call (reused vs computed English).
  - **Follow-ups ALL BUILT + PUBLISHED (2026-07-26 same day):**
    - **(a) Web badges**: guest language picker shows a "via English" chip per pivoted language (`loadPivotRoutes()` in app.js, ES5, fed by the routing endpoint; source normalised whisper→FLORES server-side); host tools panel shows the effective pivot mode line; refreshes on room-join and host language change. New CSS `.lp-lang-badge`; web.* locale keys en/ca/es.
    - **(b) Desktop preview**: `FormPivotPreview` (Options → Translation → "Preview routing…") — pick source+engine, all-languages list direct/pivot with reasons, built from the UNSAVED mode selection via the same PivotPolicy class (no drift). Localized en/ca/es.
    - **(c) Lite web settings**: pivot dropdown on the admin settings page (`#setPivot`), round-tripped through /api/settings and **live-applied** (`PivotPolicy.UpdateMode`) — no container restart.
    - **(d) FLORES-200 A/B**: benchmark "Pair A/B (FLORES)" tab — in-form download of the dev set (~26 MB, dl.fbaipublicfiles.com, verified live), own `ChrFScorer` (chrF n=1..6 β=2, corpus-level), `PairQualityRunner` runs N sentences direct (`noPivot:=True` param added to `TranslateAsync` for a policy-proof baseline) vs manual two-hop pivot, scores both vs references, shows examples. "Save as measured direct pair" writes an engine-scoped entry to **`<config-dir>/translation-direct-pairs.local.json`** — a new OVERLAY file PivotPolicy loads in addition to the shipped file (survives publishes AND Lite container updates); applies on next server start.
    - **Remaining**: ~~run the first real A/B — NLLB cat↔swe~~ (done 2026-07-26: tie 60.1/60.7 → NLLB flipped to direct-biased 2026-07-28); optional DeepL-for-Nordics via per-language overrides.
  - **✅ BUILT + PUBLISHED (2026-07-26) — CometKiwi QE (reference-free, CROSS-PAIR comparable quality scores).** chrF can't compare pairs (different rulers — proven by eng→spa 57.3 vs cat→swe 64.7 with flawless Spanish output); QE answers "which of our offered languages are weak". Pieces: `qe-server/server.py` sidecar (FastAPI, wmt22-cometkiwi-da, CPU, HF_HUB_OFFLINE at runtime, PYLOG_QE 9300-9303), `Pipeline/QeService.vb` (PythonSidecarHost pattern, port `AppConfig.QeServerPort`=5096), Download Manager "Quality Estimation" tab (torch CPU shared with MMS + unbabel-comet + gated model download; **HF token required once** — stored in `AppConfig.HuggingFaceToken`, passed via child ENV never CLI; user must accept the model license on HF first; model ~2.3 GB, non-commercial license), benchmark integration (auto-scores every Pair A/B run when installed — QE columns in results/`pair-scores.csv` (old header auto-archived)/unified CSV; sidecar starts on demand, stops with the form). **VALIDATED live on Jezer same night (2026-07-26/27):** install (token+gating) worked; one fix needed — the checkpoint's encoder TOKENIZER is a second, ungated HF repo fetched lazily at first model load, and runtime `HF_HUB_OFFLINE=1` blocked it ("Unable to load vocabulary from file") → offline flag removed + installer now does a full `load_from_checkpoint` prefetch. First scored runs (NLLB/Local, n=60, eng→X): **spa 0.853 / cat 0.843 / als 0.833** — all strong, tight spread; **Albanian cleared for offering** (the "eng→spa must beat eng→albanian" hunch was wrong on NLLB). Note the two-ruler proof in the same runs: chrF spa 57.0 vs cat 66.0 while QE says they're equal. Rough QE calibration: >0.8 strong, 0.6-0.8 usable, <0.6 investigate; CometKiwi's own judgment is softer on lower-resource targets (±0.02 = noise). NEXT: the production-direction sweep `cat_Latn→*` for all offered languages (state-of-our-languages report from pair-scores.csv); re-run cat↔swe trio to backfill empty QE columns.
...
- [ ] **Considered & shelved (record only):** *Melia* multilingual model (batch-only real-time + no Catalan yet — revisit when both land — **UPDATE 2026-07-17: revisit conditions materializing.** Melia now covers all 55+ languages incl. Catalan with automatic code-switching (the exact ca/es mixing problem), it's their cheapest tier (~$0.129/hr, 10 free hrs/mo), and the RT release notes (2026-06-28) added a `model` property accepting melia-1 — early-access, NOT confirmed GA (user: "coming, not available yet"). **When RT Melia goes GA: trial on a real service** — near-zero code thanks to v2.7.9's model dual-path; verify first that RT Melia emits END_OF_UTTERANCE + word timings (commit logic + EOU auto-tune depend on them) and that no-translation/no-auto is acceptable (it is — NLLB won the bake-off)); *diarization for attribution* (audience doesn't need "who"); *music detection / audio-events* (annotates only, unreliable on sung worship — a manual "pause during music" control is more dependable).
...
| [20](#20-translation-load-testing-suite) | Translation Load Testing Suite | **Done** (v1.8.2) | 2 |
...
**Status:** FIRST SLICE SHIPPED in v2.12.0 (2026-08-02) — a post-close feedback page for conference-room guests: 1–5 stars + free-text comment + auto-captured device (UA-derived) and viewer language, POSTed to `/api/rooms/{id}/feedback` and logged as **event 5109** in the session log (one greppable line per submission; dedupe per client; host exempt; problem-framed wording per owner — "don't phish for compliments"). Deliberate deviations from the plan below: close-triggered (not a menu button), session-LOG storage (not `feedback/session_*.json` — the log review IS the aggregation workflow today), no issue-category checkboxes yet. Still open from (a): the mid-service menu entry point, issue categories, and the **glossary "Flag a phrase" long-press** (untouched — likely the highest-value remainder); (b) operator feedback, (c) aggregation/reports, (d) JSON data format all not started.
