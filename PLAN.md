...
- [ ] **Web-mic for whisper rooms** — field finding 2026-07-29 (Jezer, room 9urxtp): a web-audio-source room on whisper-cpp silently captured LOCAL device 0 and 403d the browser frames (transcripts came from the machine mic, not the phone). live-server whisper/VAD mode never wires /audio-in into the pipeline — web-mic is effectively streaming-engines-only. Wire /audio-in frames into the VAD pipeline for local engines (relevant to the user musing: whisper+SaT possibly replacing Speechmatics needs this).
