...
## IA restructure — three-page navigation (AGREED 2026-07-17, not yet built)
...
**QR audit (2026-07-17): already room-scoped.** `/api/rooms/{id}/qr` encodes `index.html?room={id}` (with the PublicHostFor LAN-IP fix), used by both the lobby's post-create overlay and the in-room host panel — guests scanning any room QR land directly in the room. No QR format changes needed; the only adaptation is prominence (the QR overlay + host panel are the volunteer's handout tools now that the lobby isn't a guest path). Desktop FormQrCode's bare-URL QR remains the legacy-live-mode handout.
