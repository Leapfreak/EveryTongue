...
- [ ] **BEC (Bíblia Evangèlica Catalana)**: talk to Noemí Cortès about permission to include it; then catalog/import support. Vision: translation to ~every language + ad-free hassle-free scriptures on one platform. **Data bug to mention to her**: BEC book 690 `long_name` reads "2 JOAN" but 690 is FIRST John in the universal numbering (692 dup) — harmless to our alias index (short_name correct) but wrong in her source data.
- [ ] **Bible data-quality fixes (affect the derived alias index):** spavbl (Spanish) still carries ENGLISH short_names for some books plus a "Ruh" typo (Ruth) — the alias index inherits whatever the books table says, so Spanish detection quality is capped by that data until the Bible is fixed or replaced.
- [ ] **Deuterocanonical detection gap:** the detector can resolve deutero books (universal numbers 192/341/462/464 present in BCI) but the subtitle wire format has no book codes for them — a detected deutero reference currently cannot be broadcast/linked. Needs wire-code additions in BOTH Bible viewers if BCI/deutero content becomes a real use case.
...
- [x] **✅ BUILT + PUBLISHED (2026-07-26, was P1) — English-pivot for weak target languages.** (Origin: two Swedish guests abandoned translation ~9 min in — Google `cat→swe` direct was weak while `cat→eng`/`eng→swe` were strong.) **Design: one routing policy in the orchestrator — no caller anywhere changes; all heads (desktop/Lite/web) get it automatically.**
  ...
  - **Decision order (Auto)**: measured/seed entry in `wwwroot/data/translation-direct-pairs.json` (16 seed "cousin" pairs, symmetric, optional per-engine `engines` scoping; measured entries carry chrF scores+engine+date) → engine's `EnglishCentric` registry bias (google/azure/amazon/libre/nllb=True conservative; deepl/deepseek/openai=False). Pairs including the pivot language never pivot.
  ...
  - **Follow-ups ALL BUILT + PUBLISHED (2026-07-26 same day):**
    ...
    - **Remaining**: run the first real A/B — **NLLB cat↔swe** — to settle NLLB's EnglishCentric bias with evidence; optional DeepL-for-Nordics via per-language overrides.
