# EveryTongue — TODO (updated 2026-07-17, v2.8.1 — released everywhere: git, ghcr :2.8.1/:latest, GitHub Release. v2.8.0 = three-page IA restructure; **v2.8.1 = CRITICAL fix**: the IA surgery doubled `<div id="panel">` in index.html, nesting the whole room UI (captions/picker/Bible) inside the hidden settings panel — captions received+rendered but INVISIBLE. Found via field instrumentation in 4 runs (received/rendered counters → visual-state heartbeat → ancestor chain named `container[0x0] inside panel[0x0 none]`). `tools/render-test.js` (real app.js in headless Edge) now asserts PHYSICAL visibility — container parent must be BODY + nonzero on-screen rect — DOM presence alone no longer passes. The diagnostic instruments stay in the client. LESSON: scripted HTML surgery must be followed by a tag-balance check; the harness's "in DOM = rendered" assumption was the blind spot.)
