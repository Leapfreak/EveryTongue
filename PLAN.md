# EveryTongue — TODO (updated 2026-07-13, v2.6.6 — pushed)
...
- [ ] **Raw JSON config editor in web settings (~0.5 d).** PIN-gated "Advanced" textarea: GET whole config.json, validate on save, hot-apply what the settings pipeline already handles (keys/engines/PIN), flag the rest needs-restart. Blunt but covers EVERY setting from a browser immediately (admin owns the server; keys visible behind PIN is acceptable).
- [ ] **Mac one-command installer (~0.5 d).** `install/mac.sh` + double-clickable `.command` in the repo: check Docker Desktop, create config dir, `docker run` with ports/volume/EVERYTONGUE_PUBLIC_HOST, open Safari at settings. Docker Desktop install itself stays a manual once-off; true 1-click (signed native app, $99 notarization) rejected.
- [x] **LANDMINE FIXED 2026-07-14 (v2.7.11) — conversation rooms guarded when the server can't transcribe.** Conversation-room PTT does one-shot POST /transcribe (ConversationAudioHandler → live-server), which needs a transcribe-capable engine (whisper) — Lite has none, and streaming engines (Speechmatics) can't serve one-shots (same limitation hit on the desktop months ago). A conversation room in the container spins up but transcribes nothing. Options: (a) UX guard — hide/disable "Create Conversation" when no transcribe engine exists (server flag, like hasLiveSession); (b) real fix — PTT falls back to a short-lived streaming session per utterance; (c) CPU whisper in the image (scope decision, see dictation entry). DONE (a): `TranscribeCapable()` (offline whisper-family engine + model exists on disk, LIVE config aware) → `conversationRooms` flag on /api/config hides the lobby's Create Conversation block, and POST /api/rooms type=conversation returns 409 with a clear message. Also fixes the desktop case (global engine = streaming → rooms were silently dead). (b) per-utterance streaming fallback / (c) CPU whisper remain on-demand.
- [ ] **Web template editor (~2-4 d).** The structured Stage-4 debt: conference-template CRUD from the browser — name, hosting code, source language, audio source (local/web-mic + raw toggle), offered languages/display, translation engine. Removes the last reason a headless deployment needs a Windows machine.
