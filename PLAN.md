...
## Immediate TODO
- [ ] **Clone extractions, post-Sunday batch (2026-07-30, user-approved; CLONE-REPORT.md deleted after reading — this line is the surviving tracker).** Done already: PipelineRunner shared steps, LatencyStats, AppendStageCsv, TranscribeController outcome handlers. Remaining because they touch the Sunday live path: **(G2)** whisper backend base class — FasterWhisperBackend/WhisperCppBackend/CloudStreamingSttBackend share runner-forwarding boilerplate (every new capability gets pasted 2-3×); **(G3)** ConversationAudioHandler mirrors ~12 ServerOptions whisper properties (config drift by design); plus round-2 smaller ones: translation-backend per-target loop family, EndpointRegistration.Settings internals, vad pipeline↔accumulating_pipeline scaffolding, template-form combo block, sidecar log-writer ×4 (needs a packaging decision). Definition of done per group = `node tools/audit-clones.js` stops printing it.
