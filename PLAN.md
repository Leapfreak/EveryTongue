# EveryTongue — TODO (updated 2026-07-14, v2.7.15 — pushed; GitHub release page lags at v2.7.4)
...
- [ ] **Bible downloads from web settings (~0.5 d).** Lite ships Bible-free (user content + licensing: BCI et al. aren't freely redistributable — never bake into the public image). Today: copy .sqlite3 into `<config>/Bibles` (installer prints this hint). Proper fix: move the eBible.org catalog-fetch + download orchestration from FormDownloadManager into Core (UsfmConverter already lives there) and add a PIN-gated settings section listing the freely-redistributable catalog. Copyrighted translations stay manual-copy regardless.
