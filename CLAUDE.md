...
- **All user-facing strings must be localised** — never hardcode English in code. Use `GetString("Key")` or `_getString("Key")` at runtime via `LanguagePackService`. Locale files are JSON in `locales/*.json`. The number of supported languages is not fixed — any language pack can be downloaded or added. When adding new string keys, add to `locales/en.json` then translate to all other existing locale files in the folder.
  - **ENFORCED MECHANICALLY** via the audit suite (see "Audit suite" section below). When editing locale JSON by script, insert lines next to a related anchor key — never parse/re-serialize the whole file (it destroys the grouped ordering).

## Audit suite — recurring rules made mechanical (tools/audit-*.js)
- **Every publish automatically runs the exact tier**: `node tools/audit-suite.js --publish` (warn-only so publish never breaks, but **any warning printed is a must-fix before committing**). Full suite incl. heuristics, run on demand / before releases: `node tools/audit-suite.js`.
- Exact tier (zero false positives): `audit-hardcoded-strings` (English literals bypassing LanguagePackService; allowlist `tools/hardcoded-strings.allow.json`, every entry needs a written reason), `audit-locale-parity` (locale files in sync + every `GetString`/`LabelKey` key exists in en.json), `audit-es5-appjs` (app.js stays ES5), `audit-api-footgun` (`Async Function ... As Task(Of IResult)` silent-200 gotcha), `audit-code-bans` (`Debug.WriteLine`, `List(Of Object)`).
- Heuristic tier (findings = suspects for human review): `audit-language-lists` (inline language-code collections outside the canonical table/registry), `audit-pipe-drain` (pipe redirect + WaitForExit without reading both pipes), `audit-dead-code` (unreferenced Private members, partial-class-aware + LogEvents IDs never logged), `audit-clones` (12-line copy-paste blocks — extract-shared-rule candidates).
- Report tier (never pass/fail — the output IS the product): `audit-capability-matrix` (engines × capabilities table derived from code markers; an empty cell is a question — does the capability apply, and if yes why is it missing? This is how "SaT/tuning existed for Speechmatics but not whisper" stays visible. When adding a capability to ANY engine, add a column).
- **When a problem class recurs twice, don't write a reminder — write an auditor**: create `tools/audit-<name>.js` (use `tools/audit-lib.js` helpers; print findings, exit 1), register it in `tools/audit-suite.js` with a tier (`publish` only if zero-false-positive) and a one-line purpose. Allowlist entries always carry the reason.
